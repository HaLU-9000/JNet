...
def train_loop(n_epochs, optimizer, model, loss_fn, train_loader, val_loader,
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for train_data in train_loader:
            ...
            loss  = branch_calc_loss(out, rec, image, labelx,
                                     loss_fn, partial, reconstruct) # calculate losses 
            loss *= loss_weight
            if ewc is not None:
...
def pretrain_loop(n_epochs,
                  ...
                  device, path, savefig_path, model_name,
                  params, train_dataset_params,
                  scheduler=None,
                  es_patience=10,
                  is_vibrate=False,
                  wx=1.,wz=1.,
...
def finetuning_loop(n_epochs, optimizer, model, loss_fn,
                    train_loader, val_loader,
                    device, path, savefig_path, model_name,
                    ewc, train_dataset_params,
                    scheduler=None, es_patience=10,
                    is_vibrate=False,
                    loss_weight=1., ewc_weight=1000000,
                    qloss_weight = 1/100, ploss_weight = 1/100,
                    ):
    earlystopping = EarlyStopping(name        = model_name ,
                                  path        = path       ,
                                  patience    = es_patience,
