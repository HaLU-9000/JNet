...
coeff_bet_z = 10
model_names   = ['JNet_252_bet_z_10',
                 'JNet_251_bet_z_20',
                 'JNet_241_x6_largeblur-pretrain']
folderpathes  = ['_var_num_beadsdata2_10',
                 '_var_num_beadsdata2_20',
                 '_var_num_beadsdata2']
lossavgdict = {}
arr = np.array([[0., 0., 0.,],
                [0., 0., 0.,],
...
for m, model_name in enumerate(model_names):
    for f, folderpath in enumerate(folderpathes):
        ...
        val_dataset   = RandomCutDataset(folderpath  =  folderpath,  ###
        ...
        for n, val_data in enumerate(val_loader):
            ...
            if figure and n == 0:
                ...
                ax6.imshow(_label[0, :, i, :],
                        cmap='gray', vmin=0.0, vmax=1.0, aspect=1)
                plt.savefig(f'result/{model_name}_{folderpath}_result_{n}.png',
                            format='png', dpi=250)
        arr[m][f] = lossavg
        print(arr)
        lossavgdict = {**lossavgdict, **{f'{model_name}_{folderpath}':lossavg}}
sns.heatmap(arr, xticklabels=["10", "20", "30"], yticklabels=["10", "20", "30"],
            annot=True, fmt=".3f", cmap="RdYlBu_r", linewidths=2.)
plt.xlabel("bet_z in inference")
plt.ylabel("bet_z in training")
plt.title("")
plt.savefig("result/heatmap_200_2.png")
